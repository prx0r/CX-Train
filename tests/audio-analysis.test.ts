...
import {
  ...
  deleteRecording,
  listRecordings,
} from '../lib/mvp/audio/recorder';
import { generateRecordingId } from '../lib/mvp/audio/recorder';
import fs from 'fs';
...
describe('Audio Analyzer', () => {
  ...
  it('detects alternating speech and silence segments', async () => {
    ...
    assert.ok(analysis.durationMs >= 1000 && analysis.durationMs <= 1600, `duration ${analysis.durationMs}ms should be ~1300ms`);
...
describe('Audio Recorder', () => {
  ...
  const testToken = 'test-token-recorder';

  after(() => {
    const recordings = listRecordings(testToken);
    for (const r of recordings) {
      const fileName = r.fileName;
      const id = fileName.replace(`${testToken}-`, '').replace('.webm', '');
      deleteRecording(testToken, id);
    }
  });

  it('saves and retrieves a recording', () => {
    const audioData = Buffer.from('fake audio data');
    const info = saveRecording(audioData, testToken, 5000);

    assert.ok(info.id.length > 0, 'generated ID');
    assert.equal(info.assessmentToken, testToken);
    assert.equal(info.durationMs, 5000);
    assert.ok(info.sizeBytes > 0);
    assert.ok(fs.existsSync(info.filePath), 'file exists on disk');
    assert.ok(recordingExists(testToken, info.id), 'recordingExists returns true');
  });

  it('returns null stream for missing recording', () => {
    const stream = getRecordingStream(testToken, 'nonexistent');
    assert.equal(stream, null);
  });

  it('deletes a recording', () => {
    const audioData = Buffer.from('fake audio data');
    const info = saveRecording(audioData, testToken, 1000);

    assert.ok(recordingExists(testToken, info.id));

    const deleted = deleteRecording(testToken, info.id);
    assert.ok(deleted);
    assert.ok(!recordingExists(testToken, info.id));
  });

  it('returns false when deleting nonexistent recording', () => {
    const result = deleteRecording(testToken, 'nonexistent');
    assert.equal(result, false);
  });

  it('lists recordings for a token', () => {
    saveRecording(Buffer.from('data1'), testToken, 1000);
    saveRecording(Buffer.from('data2'), testToken, 2000);

    const list = listRecordings(testToken);
    assert.ok(list.length >= 2);
    assert.ok(list.every(r => r.assessmentToken === testToken));
    assert.ok(list.some(r => r.sizeBytes === 'data1'.length));
    assert.ok(list.some(r => r.sizeBytes === 'data2'.length));
  });

  it('returns empty list for unknown token', () => {
    const list = listRecordings('nonexistent-token');
