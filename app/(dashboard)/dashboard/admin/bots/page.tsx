...
export default async function AdminBotsPage() {
  ...
  const typeOrder = ['call', 'qualification', 'escalation', 'ticket', 'other'];
  const sortedTypes = typeOrder.filter((t) => botsByType[t]?.length);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="space-y-12">
        {sortedTypes.map((typeKey) => {
          const typeConfig = BOT_TYPE_CONFIG[typeKey] ?? BOT_TYPE_CONFIG.other;
          const typeBots = botsByType[typeKey] ?? [];
          ...
          return (
            <section key={typeKey}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-lg font-medium text-white">{typeConfig.label}</h2>
                  <p className="text-zinc-500 text-sm">{typeConfig.description}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {typeBots.map((bot: Bot) => {
