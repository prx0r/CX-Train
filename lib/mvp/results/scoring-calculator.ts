...
function determineEvidenceStatus(
  ...
  if (aiEvidence && aiEvidence.length > 0) {
    /* If all evidence is auto-generated (event data, keyword matches, etc.),
       treat it as verified directly — system-level events are authoritative. */
    const allAuto = aiEvidence.every(e => isAutoGenerated(e));
    if (allAuto && aiEvidence.length > 0) {
      return { status: 'verified', quote: aiEvidence[0] };
    }
