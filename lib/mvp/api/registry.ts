...
export const MVP_API_ROUTES: MvpApiRoute[] = [
  ...
  },
  {
    ...
    status: 'active',
    description: 'Send candidate message and get AI caller reply; supports text and voice metadata',
    expectedInputs: ['message', 'input_source', 'audio_metadata'],
    expectedOutputs: ['reply', 'modelUsed', 'success', 'input_source'],
  },
  {
    module: 'assess',
    method: 'GET',
    path: '/api/mvp/assessment/[token]/sim',
    status: 'active',
    description: 'Get dashboard simulation state, safe actions, visible state, and timeline',
    expectedOutputs: ['phase', 'safe_actions', 'visible_state', 'timeline'],
  },
  {
    module: 'assess',
    method: 'POST',
    path: '/api/mvp/assessment/[token]/sim/action',
    status: 'active',
    description: 'Perform dashboard simulation action and append deterministic event evidence',
    expectedInputs: ['action_id', 'tool_id', 'started_at_ms'],
    expectedOutputs: ['phase', 'safe_actions', 'visible_state', 'timeline'],
  },
  {
    module: 'voice',
    method: 'POST',
    path: '/api/mvp/assessment/[token]/voice/transcribe',
    status: 'active',
    description: 'Transcribe candidate microphone audio through STT',
    expectedInputs: ['audio', 'duration_ms'],
    expectedOutputs: ['text', 'provider', 'model', 'metadata'],
  },
  {
    module: 'voice',
    method: 'POST',
    path: '/api/mvp/assessment/[token]/voice/tts',
    status: 'active',
    description: 'Synthesize customer reply audio through TTS',
    expectedInputs: ['text', 'voice'],
    expectedOutputs: ['audio/mpeg'],
  },
  {
    module: 'assess',
    method: 'POST',
