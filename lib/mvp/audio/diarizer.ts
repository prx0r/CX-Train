/**
 * Speaker diarization using sherpa-onnx-node.
 *
 * Separates a mixed audio recording into per-speaker segments,
 * labeling each segment as "candidate" or "customer".
 *
 * Runs entirely on CPU via ONNX — no GPU needed.
 */
...
export async function runDiarization(
  ...
  try {
    ...
    // Assign labels: first speaker = customer (AI), second = candidate
