/**
 * Speaker turn tracking and timing analysis.
 *
 * Reads session_events to build a timeline of who spoke when,
 * computing pause times, response latency, and talk ratios.
 */
...
export interface TurnTimelineSummary {
  totalTurns: number;
  avgResponseLatencyMs: number;
  maxPauseMs: number;
  talkBalance: number;
  candidateRushScore: number;
}

export function summarizeTimeline(timeline: TurnTimeline): TurnTimelineSummary {
  const pauseTimes: number[] = [];
  for (let i = 1; i < timeline.turns.length; i++) {
    const gap = timeline.turns[i].startMs - timeline.turns[i - 1].endMs;
    if (gap > 0) pauseTimes.push(gap);
  }

  const avgPause = pauseTimes.length > 0
    ? Math.round(pauseTimes.reduce((a, b) => a + b, 0) / pauseTimes.length)
    : 0;
  const maxPause = pauseTimes.length > 0 ? Math.max(...pauseTimes) : 0;

  const total = timeline.totalCandidateTalkMs + timeline.totalCustomerTalkMs;
  const talkImbalance = total > 0
    ? Math.abs(timeline.candidateTalkRatio - timeline.customerTalkRatio)
    : 0;

  const rushScore = timeline.avgResponseLatencyMs < 300
    ? 10
    : timeline.avgResponseLatencyMs > 5000
      ? 1
      : Math.round(10 - (timeline.avgResponseLatencyMs / 5000) * 9);

  return {
    totalTurns: timeline.totalCandidateTurns + timeline.totalCustomerTurns,
    avgResponseLatencyMs: timeline.avgResponseLatencyMs,
    maxPauseMs: maxPause,
    talkBalance: talkImbalance,
    candidateRushScore: Math.max(1, Math.min(10, rushScore)),
  };
}
